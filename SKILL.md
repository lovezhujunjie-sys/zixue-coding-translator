---
name: 自学编程
description: 开发与迭代「代码翻译官」零基础自学编程 web 应用（让完全没接触过代码的小白，从看懂到会写）。单文件 index.html，纯前端 + localStorage，含六大板块：代码是啥（概念打比方）、逐行翻译（点代码看人话）、黑话词典（36 关键词分类速查）、自己动手写（带半成品的填空练习 + 内置迷你 Python 解释器 v2 + 变量盒子可视化）、小测验、自由练习场（任意写代码即时运行）。触发场景：用户想新增/优化这个学编程应用的功能、加例子、加练习题、扩展迷你解释器、改文案视觉、做进度记忆、或部署分享。每次改完都要：①更新本地 skill 文件 ②commit 并 push 到 GitHub。
---
